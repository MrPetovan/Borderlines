...
class Attack extends Player_Order {
  ...
  public function pre_execute() {
    ...
    $attacking_player->set_player_resource_history( $attacking_player->current_game->id, 2, $attacking_player->current_game->current_turn - 1, mktime(), - $soldiers_sent, $message, $this->id );
  }
  ...
  public function execute() {
    ...
    if( isset( $parameters['count'] ) && isset( $parameters['player_id'] ) ) {
      ...
      if( $defending_player ) {
        ...
        $attacking_player->set_player_resource_history( $game_id, 2, $resource_turn, mktime(), $soldiers_returned, $attacker_message, $this->get_id() );
        $attacking_player->set_player_resource_history( $game_id, 4, $resource_turn, mktime(), $territory_gained, $attacker_message, $this->get_id() );
        $defending_player->set_player_resource_history( $game_id, 2, $action_turn, mktime(), - $defender_losses, $defender_message, $this->get_id() );
        $defending_player->set_player_resource_history( $game_id, 4, $action_turn, mktime(), - $territory_gained, $defender_message, $this->get_id() );
