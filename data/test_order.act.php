    ...
    switch( $_GET['action'] ) {
      case "init_db" : {
        ...
        foreach( $players as $player ) {
          ...
          $player->set_player_resource_history( 1, guess_time( time(), GUESS_DATE_MYSQL), 10000, "Init" );
          $player->set_player_resource_history( 2, guess_time( time(), GUESS_DATE_MYSQL), 1000, "Init" );
          $player->set_player_resource_history( 3, guess_time( time(), GUESS_DATE_MYSQL), 1000, "Init" );

          $order = Player_Order::instance();
          $order->set_order_type_id( 2 );
          $order->set_player_id( $player->get_id() );
          $order->set_datetime_order( guess_time( time(), GUESS_DATE_MYSQL) );
          $order->set_datetime_scheduled( guess_time( time(), GUESS_DATE_MYSQL) );
